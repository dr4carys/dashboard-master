...
const readKabupaten = async () => {
  startLoading();
  const req = await fetch("https://api.sipandu-beradat.id/kabupaten/");
  ...
  if (status_code === 200) {
    setupFilterDataTable(
      ...
      data.map((obj, i) => [
        ...
        data-target="#modal-edit-kabupaten" data-id="${obj.id}" data-name="${obj.name}" data-status="${obj.active_status}">
        ...
        <a href="#" class="btn btn-inverse-primary-red btn-rounded btn-icon btn-action mr-2 btn-delete" title="Delete" data-toggle="modal"
            data-target="#modal-hapus-kabupaten" data-id="${obj.id}">
